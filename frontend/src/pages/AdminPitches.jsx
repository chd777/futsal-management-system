...
export default function AdminPitches() {
  ...
  const emptyForm = {
    name: "", address: "", pricePerHour: "", openTime: "06:00", closeTime: "22:00", lat: "", lng: ""
  };
  ...
  function startEdit(p) {
    setEditId(p._id);
    setForm({
      name: p.name, address: p.address, pricePerHour: p.pricePerHour,
      openTime: p.openTime, closeTime: p.closeTime, lat: p.lat, lng: p.lng
    });
    setLocQuery("");
    setLocResults([]);
    setLocMsg("");
  ...
  // Search location using free Nominatim API
  ...
  function selectLocation(item) {
    ...
    setLocResults([]);
    setLocMsg("Location set! Lat: " + parseFloat(item.lat).toFixed(6) + ", Lng: " + parseFloat(item.lon).toFixed(6));
  }
  ...
  return (
    <div>
      ...
      <p className="muted mt-sm">Add, edit, enable/disable pitches with pricing and location.</p>
      ...
      <div className="panel mt-md">
        ...
        <form onSubmit={onSubmit} className="form">
          <label>
            Pitch Name
            <input name="name" value={form.name} onChange={onChange} required placeholder="e.g. Kathmandu Futsal Arena" />
          </label>

          <label>
            Address
            <input name="address" value={form.address} onChange={onChange} required placeholder="e.g. Baneshwor, Kathmandu" />
          </label>

          <div className="grid2">
            <label>
              Price per Hour (NPR)
              <input name="pricePerHour" type="number" min="0" value={form.pricePerHour} onChange={onChange} required />
            </label>
            <label>
              Open Time
          ...
          <div className="panel" style={{ background: "rgba(91,140,255,0.05)", border: "1px solid rgba(91,140,255,0.2)" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              />
              <button type="button" className="btn" onClick={searchLocation} disabled={locSearching}>
            ...
            {locResults.length > 0 && (
              <div style={{ marginTop: 10 }}>
                <p className="muted small" style={{ marginBottom: 6 }}>Click to select a location:</p>
            ...
            {locMsg && (
              <p className={locMsg.includes("set") ? "text-ok" : "muted"} style={{ marginTop: 8, fontSize: 13 }}>
                {locMsg}
              </p>
            )}
            ...
            <div className="grid2" style={{ marginTop: 10 }}>
              <label>
                Latitude
                <input name="lat" type="number" step="0.000001" value={form.lat} onChange={onChange} required placeholder="Auto-filled" />
              </label>
              <label>
                Longitude
                <input name="lng" type="number" step="0.000001" value={form.lng} onChange={onChange} required placeholder="Auto-filled" />
              </label>
            </div>

            {/* Mini map preview */}
            {form.lat && form.lng && (
              <div style={{ marginTop: 10, borderRadius: 10, overflow: "hidden", border: "1px solid var(--border)", height: 200 }}>
                <iframe
      ...
      {loading ? (
        ...
        <div className="list mt-sm">
          {pitches.map(p => (
            <div key={p._id} className="list-item" style={{ flexDirection: "column", alignItems: "stretch" }}>
              <div className="flex-between">
                <div>
                  ...
                  <p className="muted small mt-sm">
                    NPR {p.pricePerHour}/hr &middot; {p.openTime} - {p.closeTime} &middot; ({p.lat}, {p.lng})
                  </p>
                </div>
                <div className="flex-gap">
                  ...
                  <button className="btn small danger" onClick={() => onDelete(p._id)}>Delete</button>
                </div>
              </div>
            </div>
          ))}
