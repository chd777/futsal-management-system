...
export default function MyBookings() {
  ...
  async function loadBookings() {
    ...
    try {
      ...
      for (const r of reviews) {
        if (r.booking) reviewedBookingIds.add(r.booking._id || r.booking);
      }
  ...
  function sortSlots(items) { return [...items].sort((a, b) => a.slot.localeCompare(b.slot)); }
  ...
  const groupedBookings = useMemo(() => {
    ...
    return Array.from(map.entries())
      .map(([groupKey, items]) => {
        ...
        const totalPrice = sortedItems.reduce((sum, item) => sum + Number(item.priceAtBooking || 0), 0);
        return {
          groupKey, representativeId: first._id,
          bookingGroup: first.bookingGroup || null,
          pitch: first.pitch, date: first.date, status: first.status,
          refundStatus: first.refundStatus, cancelReason: first.cancelReason,
          ...
          bookings: sortedItems, totalHours: sortedItems.length,
  ...
  const upcomingList = groupedBookings.filter(b => b.date >= today && b.status !== "CANCELLED");
  const pastList = groupedBookings.filter(b => b.date < today || b.status === "CANCELLED");

  const filteredBase = tab === "upcoming" ? upcomingList : tab === "past" ? pastList : groupedBookings;
  const filtered = filteredBase.filter(b => {
    const matchesSearch = b.pitch?.name?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  async function cancelBookingGroup(group) {
    ...
    try {
      await Promise.all(group.bookingIds.map(id => api.put(`/api/bookings/${id}/cancel`)));
      loadBookings();
  ...
  async function handleKhaltiPayment(group) {
    try {
      const res = await api.post("/api/payments/khalti/initiate", { bookingId: group.representativeId });
      if (res.data?.payment_url) window.location.href = res.data.payment_url;
      else alert("Payment URL not received.");
    } catch (e) { alert(e?.response?.data?.message || "Failed to initiate payment"); }
  }

  async function payLater(group) {
    if (!confirm("Confirm booking with Pay Later? You'll need to pay with cash at the venue.")) return;
    try {
      const res = await api.post("/api/payments/pay-later", { bookingId: group.representativeId });
  ...
  async function submitReview() {
    ...
    try {
      await api.post("/api/reviews", { bookingId: reviewModal.representativeId, rating: reviewRating, comment: reviewComment });
  ...
  function statusPill(status, isLoyalty, refundStatus) {
    ...
    if (status === "CANCELLED" && refundStatus === "REFUNDED") return <span className="pill cancelled">Cancelled & Refunded</span>;
  ...
  function clearFilters() { setSearchTerm(""); setStatusFilter("ALL"); setDateFilter(""); }

  const dateInputStyle = { colorScheme: "dark", minWidth: 160, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--border)", background: "#0f1622", color: "#eef2f7", fontSize: 14, outline: "none" };
  ...
  return (
    <div>
      ...
      <div className="tabs mt-md">
        <button className={`tab ${tab === "upcoming" ? "active" : ""}`} onClick={() => setTab("upcoming")}>Upcoming ({upcomingList.length})</button>
        <button className={`tab ${tab === "past" ? "active" : ""}`} onClick={() => setTab("past")}>Past ({pastList.length})</button>
        <button className={`tab ${tab === "all" ? "active" : ""}`} onClick={() => setTab("all")}>All ({groupedBookings.length})</button>
      </div>

      <div className="filter-bar mt-md" style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
        <label>
          Search Pitch
          <input type="text" placeholder="Search by pitch name..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </label>
        <label>
          Status
        ...
        <label>
          Date
          <input type="date" value={dateFilter} onChange={e => setDateFilter(e.target.value)} style={dateInputStyle} />
        </label>
        {hasFilters && <button className="btn small ghost" onClick={clearFilters} style={{ marginBottom: 1 }}>Clear Filters</button>}
      </div>
      ...
      {loading ? (
        ...
        <div className="list">
          {filtered.map(g => (
            <div key={g.groupKey} className="list-item" style={{ flexDirection: "column", alignItems: "stretch" }}>
              <div className="flex-between">
                <div>
                  ...
                  <div className="muted small mt-sm">
                    ...
                    {g.isLoyaltyReward ? (
                      <span style={{ color: "var(--ok)", fontWeight: 700 }}>FREE (Loyalty)</span>
                    ) : (
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                  ...
                  <div className="flex-gap">
                    {g.status === "PENDING_PAYMENT" && (
                      <>
                        <button className="btn small ok" onClick={() => handleKhaltiPayment(g)}>Pay Now</button>
                        <button className="btn small warn" onClick={() => payLater(g)}>Pay Later</button>
                        <button className="btn small danger" onClick={() => cancelBookingGroup(g)}>Cancel</button>
                      </>
                    )}
                    {(g.status === "PAID" || g.status === "CONFIRMED_PAY_LATER") && g.date <= today && !g.isReviewed && (
                      <button className="btn small" onClick={() => { setReviewModal(g); setReviewRating(5); setReviewComment(""); }}>Write Review</button>
                    )}
                    {g.isReviewed && <span className="pill active" style={{ background: "rgba(59,130,246,0.15)" }}>⭐ Reviewed</span>}
                  </div>
                </div>
              </div>
            </div>
      ...
      {reviewModal && (
        <div className="modal-overlay" onClick={() => setReviewModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <p className="muted mt-sm">{reviewModal.pitch?.name} &middot; {formatDate(reviewModal.date)}</p>
            <div className="mt-md">
            ...
            <div className="flex-gap mt-md">
              <button className="btn ok" onClick={submitReview} disabled={reviewLoading}>{reviewLoading ? "Submitting..." : "Submit Review"}</button>
