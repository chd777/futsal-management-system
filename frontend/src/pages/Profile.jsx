...
const Icons = {
  ...
  eyeOff: (p) => <Icon {...p} d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19M1 1l22 22M14.12 14.12a3 3 0 1 1-4.24-4.24" />,
...
const S = {
  ...
  accent: {
    background: "linear-gradient(135deg, #00d296, #0ea5e9)",
  ...
  heroGlow: {
    ...
    width: 160,
    height: 160,
    background: "radial-gradient(circle, rgba(0,210,150,0.1), transparent 70%)",
    pointerEvents: "none",
  },
  avatar: {
    width: 84,
    ...
    borderRadius: "50%",
    objectFit: "cover",
    border: "3px solid rgba(0,210,150,0.4)",
    boxShadow: "0 0 24px rgba(0,210,150,0.15)",
  },
  avatarFallback: {
    width: 84,
    height: 84,
  ...
  cameraBadge: {
    ...
    alignItems: "center",
    justifyContent: "center",
    border: "2.5px solid var(--bg, #0c1220)",
    cursor: "pointer",
  ...
  saveBtn: {
    ...
    letterSpacing: "0.3px",
    transition: "all 0.3s",
    boxShadow: "0 4px 20px rgba(0,210,150,0.18)",
  },
  editBtn: {
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.content}>
        {/* ── Title ── */}
        <h1 style={S.title}>
          My <span style={S.accent}>Profile</span>
        </h1>
        <p className="muted" style={{ fontSize: 14 }}>View and manage your account settings</p>
        ...
        <div style={S.heroCard}>
          ...
          <div style={{ position: "relative", flexShrink: 0 }}>
            ...
            <label style={S.cameraBadge}>
              {photoUploading
                ? <span style={{ fontSize: 10, color: "#0a0e17" }}>...</span>
                : <Icons.camera size={13} color="#0a0e17" />
              }
              <input
                type="file"
                accept="image/*"
          ...
          <div style={{ flex: 1, minWidth: 180 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {isGoogleUser && (
                <span style={S.googleBadge}>G Google Account</span>
              )}
              {profile.profilePicture && profile.cloudinaryId && (
                <button onClick={removePhoto} style={S.removeBadge}>
                  <Icons.trash size={12} /> Remove Photo
                </button>
              )}
            </div>
          </div>
          ...
          {!editing && (
            <button
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.borderColor = "rgba(0,210,150,0.25)";
                e.currentTarget.style.color = "#00d296";
              }}
              onMouseLeave={(e) => {
        ...
        <div style={S.statsRow}>
          <StatCard
            icon={<Icons.game size={20} color="#00d296" />}
            value={stats.total}
            label="Total Bookings"
            accent="#00d296"
          />
          <StatCard
            icon={<Icons.check size={20} color="#6366f1" />}
            value={stats.paid}
            label="Completed"
            accent="#6366f1"
          />
          <StatCard
            icon={<Icons.calendar size={20} color="#0ea5e9" />}
            value={stats.upcoming}
            label="Upcoming"
            accent="#0ea5e9"
          />
          <StatCard
            icon={<Icons.money size={20} color="#f59e0b" />}
            value={`Rs. ${stats.total > 0 ? Math.round(stats.paid * 1500 / 1000) + "K" : "0"}`}
            label="Est. Spent"
            accent="#f59e0b"
          />
        </div>
                ...
                <div style={S.fieldRow}>
                  <div style={S.fieldGroup}>
                    ...
                    <input
                      ...
                      required
                      onFocus={e => e.target.style.borderColor = "rgba(0,210,150,0.4)"}
                      onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.1)"}
                    />
                  </div>
                  <div style={S.fieldGroup}>
                    <label style={S.inputLabel}>Email</label>
                    <input style={{ ...S.input, opacity: 0.4, cursor: "not-allowed" }} value={profile.email} disabled />
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", marginTop: 4, display: "block" }}>Email cannot be changed</span>
                  </div>
                </div>

                <div style={S.fieldRow}>
                  <div style={S.fieldGroup}>
                    <label style={S.inputLabel}>College / University</label>
                    <input
                      style={S.input}
          ...
          <div style={S.rightCol}>
            ...
            <div style={S.sidePanel}>
              ...
              <div style={S.bookingRow}>
                <span style={S.bookingLabel}>Total Bookings</span>
                <span style={S.bookingValue("#6366f1")}>{stats.total}</span>
              </div>
              <div style={S.bookingRow}>
                <span style={S.bookingLabel}>Completed / Paid</span>
                <span style={S.bookingValue("#00d296")}>{stats.paid}</span>
              </div>
              <div style={{ ...S.bookingRow, borderBottom: "none" }}>
                <span style={S.bookingLabel}>Upcoming</span>
                <span style={S.bookingValue("#0ea5e9")}>{stats.upcoming}</span>
              </div>
            </div>
            ...
            <div style={S.sidePanel}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: showPassword ? 16 : 8 }}>
                ...
                {!isGoogleUser && (
                  <button
                    ...
                    onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(0,210,150,0.3)"}
              ...
              {isGoogleUser ? (
                <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 13, lineHeight: 1.6 }}>
                  Your account uses Google login. Password is managed by Google.
                </p>
              ) : showPassword ? (
                <div>
                  ...
                  <button style={{ ...S.saveBtn, fontSize: 13 }} onClick={changePassword} disabled={passLoading}>
