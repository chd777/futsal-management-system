...
/* ── group bookings (same logic as MyBookings/Home) ── */
...
export default function Profile() {
  ...
  async function loadProfile() {
    ...
    try {
      ...
      setStats({
        ...
        upcoming: grouped.filter(g => g.date >= today && g.status !== "CANCELLED").length,
        totalSpent,
      });
  ...
  async function saveProfile(e) {
    e.preventDefault();
    setEditErr(""); setEditMsg(""); setSaving(true);
    try {
    ...
    } catch (err) { setEditErr(err?.response?.data?.message || "Update failed"); }
  ...
  async function handlePhotoUpload(e) {
    ...
    try {
      ...
      const res = await api.put("/api/profile/photo", formData, { headers: { "Content-Type": "multipart/form-data" } });
      setProfile(prev => ({ ...prev, profilePicture: res.data.profilePicture }));
    } catch (err) { alert(err?.response?.data?.message || "Photo upload failed"); }
  ...
  async function changePassword(e) {
    e.preventDefault();
    setPassErr(""); setPassMsg(""); setPassLoading(true);
    try {
    ...
    <div>
      ...
      <div className="grid4 mt-md">
        ...
        <div className="stat">
          ...
          <div className="stat-value" style={{ color: "var(--ok)" }}>{stats.paid}</div>
        </div>
        <div className="stat">
          <div className="stat-title">Upcoming</div>
          <div className="stat-value" style={{ color: "var(--accent)" }}>{stats.upcoming}</div>
        </div>
        <div className="stat">
          <div className="stat-title">Total Spent</div>
          <div className="stat-value" style={{ color: "var(--warn)", fontSize: 20 }}>NPR {stats.totalSpent.toLocaleString()}</div>
        </div>
      </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
            <div style={{ position: "relative" }}>
              ...
              <label style={{
                ...
                display: "flex", alignItems: "center", justifyContent: "center",
                cursor: photoUploading ? "wait" : "pointer", fontSize: 14,
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
                ...
                {profile.profilePicture && profile.cloudinaryId && (
                  <button onClick={removePhoto} className="pill" style={{ cursor: "pointer", background: "rgba(255,91,110,0.1)", borderColor: "rgba(255,91,110,0.3)", color: "var(--danger)" }}>
                    Remove Photo
                  </button>
                )}
              </div>
          ...
          {!editing ? (
            <>
              <div style={{ display: "grid", gap: 12 }}>
                <div><div className="muted small">Full Name</div><div style={{ fontWeight: 600, marginTop: 4 }}>{profile.fullName}</div></div>
                <div><div className="muted small">Email</div><div style={{ fontWeight: 600, marginTop: 4 }}>{profile.email}</div></div>
                <div><div className="muted small">College/University</div><div style={{ fontWeight: 600, marginTop: 4 }}>{profile.college || "Not specified"}</div></div>
                <div><div className="muted small">Role</div><div style={{ fontWeight: 600, marginTop: 4, textTransform: "capitalize" }}>{profile.role}</div></div>
                <div><div className="muted small">Member Since</div><div style={{ fontWeight: 600, marginTop: 4 }}>{new Date(profile.createdAt).toLocaleDateString()}</div></div>
              </div>
            ...
            <form onSubmit={saveProfile} className="form">
              ...
              <label>Full Name<input value={editForm.fullName} onChange={e => setEditForm(p => ({ ...p, fullName: e.target.value }))} required /></label>
              <label>Email<input value={profile.email} disabled style={{ opacity: 0.5 }} /><span className="muted small">Email cannot be changed</span></label>
              <label>College/University<input value={editForm.college} onChange={e => setEditForm(p => ({ ...p, college: e.target.value }))} /></label>
              <div className="flex-gap">
                <button className="btn" disabled={saving}>{saving ? "Saving..." : "Save Changes"}</button>
                <button type="button" className="btn ghost" onClick={() => { setEditing(false); setEditErr(""); }}>Cancel</button>
              </div>
            </form>
          )}
        </div>
        ...
        <div>
          <div className="panel mb-md">
            ...
            <div style={{ display: "grid", gap: 10 }}>
              <div className="flex-between"><span className="muted">Total Bookings</span><span style={{ fontWeight: 700 }}>{stats.total}</span></div>
              <div className="flex-between"><span className="muted">Completed/Paid</span><span style={{ fontWeight: 700, color: "var(--ok)" }}>{stats.paid}</span></div>
              <div className="flex-between"><span className="muted">Upcoming</span><span style={{ fontWeight: 700, color: "var(--accent)" }}>{stats.upcoming}</span></div>
              <div className="flex-between" style={{ borderTop: "1px solid var(--border)", paddingTop: 10 }}>
                <span className="muted">Total Spent</span>
                <span style={{ fontWeight: 800, color: "var(--warn)" }}>NPR {stats.totalSpent.toLocaleString()}</span>
              </div>
            </div>
          </div>
          ...
          <div className="panel">
            ...
            {isGoogleUser ? (
              ...
              <form onSubmit={changePassword} className="form">
                ...
                <label>Current Password<input type="password" value={passForm.currentPassword} onChange={e => setPassForm(p => ({ ...p, currentPassword: e.target.value }))} required /></label>
                <label>New Password<input type="password" value={passForm.newPassword} onChange={e => setPassForm(p => ({ ...p, newPassword: e.target.value }))} placeholder="Min 8 characters" required /></label>
                <label>Confirm New Password<input type="password" value={passForm.confirmNewPassword} onChange={e => setPassForm(p => ({ ...p, confirmNewPassword: e.target.value }))} required /></label>
                <button className="btn" disabled={passLoading}>{passLoading ? "Changing..." : "Update Password"}</button>
              </form>
            ) : (
